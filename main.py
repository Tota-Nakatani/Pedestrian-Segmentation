...
def load_X(folder_path):
    import os, cv2
    #cv2のresizeの引数は（w、h)でありch方向を無視していることに注意!!
    image_files = os.listdir(folder_path)
    image_files.sort()
    images = np.zeros((len(image_files), IMAGE_SIZE, IMAGE_SIZE, 3), np.float32)#(画像数,size,size,3)のゼロtensorを生成
    for i, image_file in enumerate(image_files):
        image = cv2.imread(folder_path + os.sep + image_file)
        image = cv2.resize(image, (IMAGE_SIZE, IMAGE_SIZE))#256,256にする
...
def load_Y(folder_path):
    ...
    image_files = os.listdir(folder_path)
    image_files.sort()
    images = np.zeros((len(image_files), IMAGE_SIZE, IMAGE_SIZE, 1), np.float32)#(画像数,size,size,1)
    for i, image_file in enumerate(image_files):
        image = cv2.imread(folder_path + os.sep + image_file, cv2.IMREAD_GRAYSCALE)
        image = cv2.resize(image, (IMAGE_SIZE, IMAGE_SIZE))#256,256にする
        image = image[:, :, np.newaxis]#ch方向を
...
def train_unet():
    ...
    X_train, file_names = load_X('trainingData' + os.sep + 'left_images')#os.sep:/のこと
