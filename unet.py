...
class UNet(object):
    '''
    input_image=(256,256,3):3chのRGB画像
    output_image=(256,256,1)：0,1の値を持つ画像と同じサイズの出力
    concatenate_axis:skip-connectionで結合する軸,-1なのでch方向にconcatenateする
    first_layer_filter_count:畳み込みフィルタの数(ここではN=64)
    ...
    def __init__(self, input_channel_count, output_channel_count, first_layer_filter_count):
        ...
        dec1 = concatenate([dec1, enc7], axis=self.CONCATENATE_AXIS)#encode-decodeで対象にあるマップをch方向でconcateする
    ...
    def _add_decoding_layer(self, filter_count, add_drop_layer, sequence):
        ...
        mapに対して
        ReLU
        Conv2DTranspose(逆畳み込み、アップサンプリング)
        bn
        dropout==truであれば0.5のdropoutを行う
        によるdecodingを行う関数
